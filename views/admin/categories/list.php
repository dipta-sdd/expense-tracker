...
<div class="wrap expense-tracker-warp">
    <h1 class="wp-heading-inline"><?php esc_html_e('Categories', 'expense-tracker'); ?></h1>
    ...
    <div class="postbox" style="padding: 15px; margin-top: 15px;">
        <form id="add-category-form" method="post">
            ...
            <h3 id="add-category-title"><?php esc_html_e('Add Category', 'expense-tracker'); ?></h3>
            <table class="form-table">
                <tr>
                    <th scope="row">
                        <label for="category_name"><?php esc_html_e('Name', 'expense-tracker'); ?></label>
                    </th>
                    <td>
                ...
                <tr>
                    <th scope="row">
                        <label for="category_description"><?php esc_html_e('Description', 'expense-tracker'); ?></label>
                    </th>
                    <td>
                ...
                <tr>
                    <th scope="row">
                        <label for="category_budget"><?php esc_html_e('Budget', 'expense-tracker'); ?></label>
                    </th>
                    <td>
            ...
            <?php submit_button(esc_html__('Add Category', 'expense-tracker'), 'primary', 'submit', false); ?>
            <button type="button" id="cancel-edit" class="button" style="display:none;"><?php esc_html_e('Cancel Edit', 'expense-tracker'); ?></button>
        </form>
    </div>

    <div class="postbox" style="padding: 15px; margin-top: 15px;">
        <h3><?php esc_html_e('Existing Categories', 'expense-tracker'); ?></h3>
        <table class="wp-list-table widefat fixed striped">
            <thead>
                <tr>
                    <th scope="col"><?php esc_html_e('Name', 'expense-tracker'); ?></th>
                    <th scope="col"><?php esc_html_e('Description', 'expense-tracker'); ?></th>
                    <th scope="col"><?php esc_html_e('Budget', 'expense-tracker'); ?></th>
                    <th scope="col"><?php esc_html_e('Count', 'expense-tracker'); ?></th>
                    <th scope="col"><?php esc_html_e('Created By', 'expense-tracker'); ?></th>
                    <th scope="col"><?php esc_html_e('Updated By', 'expense-tracker'); ?></th>
                    <th scope="col"><?php esc_html_e('Created At', 'expense-tracker'); ?></th>
                    <th scope="col"><?php esc_html_e('Updated At', 'expense-tracker'); ?></th>
                    <th scope="col"><?php esc_html_e('Actions', 'expense-tracker'); ?></th>
                </tr>
            </thead>
            <tbody id="category-list">
                <?php if ($categories) : ?>
                    <?php foreach ($categories as $category) : ?>
                        <tr data-id="<?php echo esc_attr($category['id']); ?>">
                            ...
                            <td><?php echo esc_attr(wp_date(get_option('date_format') . ' ' . get_option('time_format'), strtotime($category['created_at']))); ?></td>
                            <td><?php echo esc_attr(wp_date(get_option('date_format') . ' ' . get_option('time_format'), strtotime($category['updated_at']))); ?></td>
                            <td>
                ...
                <?php else : ?>
                    <tr>
                        <td colspan="9"><?php esc_html_e('No categories found.', 'expense-tracker'); ?></td>
                    </tr>
                <?php endif; ?>
            </tbody>
        </table>
...
<script>
    jQuery(document).ready(function($) {
        ...
        $('#add-category-form').on('submit', function(e) {
            ...
            let method = 'POST';
            let url = '<?php echo esc_url(get_rest_url(null, 'expense-tracker/v1/categories')); ?>';

            if (categoryId) {
                method = 'PUT';
                url = '<?php echo esc_url(get_rest_url(null, 'expense-tracker/v1/categories/')); ?>' + categoryId;
            }
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    if (categoryId) {
                        ...
                        $('#cancel-edit').hide();
                        $('#add-category-form').find('input[type="submit"]').val('<?php echo esc_html_e('Add Category', 'expense-tracker'); ?>');
                        $('#add-category-title').text('<?php echo esc_html_e('Add Category', 'expense-tracker'); ?>');
                    } else {
                        ...
                        const newRow = $(`<tr data-id="${newCategory.id}">
                                        ...
                                        <td>
                                            <a href="#" class="edit-category" data-id="${newCategory.id}">
                                                <?php echo esc_html_e('Edit', 'expense-tracker'); ?>
                                            </a> |
                                            <a href="#" class="delete-category" data-id="${newCategory.id}">
                                                <?php echo esc_html_e('Delete', 'expense-tracker'); ?>
                                            </a>
                                        </td>
                                    </tr>`);
        ...
        $(document).on('click', '.edit-category', function(e) {
            ...
            $('#add-category-title').text('<?php echo esc_html_e('Edit Category', 'expense-tracker'); ?>');
            ...
            $('#cancel-edit').show();
            $('#add-category-form').find('input[type="submit"]').val('<?php echo esc_html_e('Update Category', 'expense-tracker'); ?>');
        });
        ...
        function formatDateTime(dateTime) {
            let dateFormat = '<?php echo esc_js(get_option('date_format')); ?>';
            let timeFormat = '<?php echo esc_js(get_option('time_format')); ?>';
